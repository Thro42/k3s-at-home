        ...
        if ($ComputerName -ne "") {
            ...
            if ($objCombobox2.SelectedItem) {
                ...
                if ($DriveName -ne "") {
                    ...
                    $outfile = -join ($outpath , 'firstrun.sh')
                    ...
                    switch ($NodeSettings.os) {
                        'buster' {
                            ...
                            $ipfile = -join ($outpath , 'interfaces.net')
                            Write-Host 'Prepare' $ipfile 'for' $IPAdress 'on' $NodeSettings.networ
                            switch ($NodeSettings.network) {
                                'eth' {  
                                    $interface = Get-Content templates\interfaces.eth
